    ...
    <div className="bg-white rounded-lg overflow-hidden shadow-md transition-transform duration-300 hover:shadow-lg hover:-translate-y-1">
      ...
      <div className="p-4">
        <Link href={`/product/${slug}`} className="block">
          <h3 className="text-lg font-semibold text-gray-900 mb-2 hover:text-blue-600">{title}</h3>
        </Link>
        <p className="text-sm text-gray-600 mb-4">{category}</p>
        
        <div className="flex flex-col space-y-2">
          <Link 
            href={`/inquire?product=${id}`}
          ...
          <a 
            href={`https://wa.me/?text=I'm interested in ${title}`}
            target="_blank"
